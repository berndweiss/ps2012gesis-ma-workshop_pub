...
\begin{frame}\frametitle{Beispiel Fisher Z-Transformation}
  ...
  \item Fisher Z-transformierte Korrelation: $z_r = 0.5 \cdot
    \ln(\frac{1+0.13}{1-0.13})$ = \Sexpr{(x <-0.5*log((1+0.13)/(1-0.13)))}
  \item Standardfehler:
    $SE_{z_r}=\frac{1}{\sqrt{123-3}}$=\Sexpr{1/(sqrt(123-3))}
  \item Rücktransformation: $r=\frac{e^{2 \cdot 0.1307} -1}{e^{2 \cdot 0.1307} +
      1}$ = \Sexpr{(exp(2*x) -1 )/(exp(2*x)+1)}
  \end{itemize}
...
\begin{frame}[plain]
  ...
  \begin{itemize}
  \item $t$-Statistik: 0.54/0.28=\Sexpr{(t.rsp <- round(0.54/0.28, digits=2))}
  \item $r_Y^2$ (R-square): \Sexpr{(R2.rsp <- 0.33)}
  \item N (nicht in Tabelle, siehe Fußnote)\footnote{Aloe \& Becker verwenden
    ...
    \Sexpr{formatC((N.rsp <- 26435), format="f", digits=0)}
  ...
  \begin{equation*}
    r_{sp}=\frac{1.9\sqrt{(1-0.33)}}{\sqrt{(26435-26-1)}}=\Sexpr{(rsp <- (t.rsp*sqrt(1-R2.rsp))/(sqrt(N.rsp-26-1)))}
  \end{equation*}
...
\begin{frame}
  ...
  \begin{equation*}
    r^2_{Y(f)}=r^2_Y - r^2_{sp} = \Sexpr{R2.rsp} - \Sexpr{rsp}^2 = \Sexpr{(R2f.rsp <- R2.rsp - rsp^2)}
  \end{equation*}
  
  Für die Varianz von $r_{sp}$ gilt dann:
  
  \begin{equation*}
    V_{r_{sp}} = \frac{\Sexpr{R2.rsp^2} - 2 \cdot \Sexpr{R2.rsp} + \Sexpr{R2f.rsp} + 1 - \Sexpr{R2f.rsp^2}}{26435} = 
    \Sexpr{formatC((V.rsp <- (R2.rsp^2 - 2 * R2.rsp + R2f.rsp + 1 - R2f.rsp^2)/(N.rsp)), format="f", digits=6)} 
  \end{equation*}
    
  Der Standardfehler beträgt:
  
  \begin{equation*}
    SE_{r_{sp}}=\Sexpr{sqrt(V.rsp)}
  \end{equation*}
